    ...
    const getAll = async () => {
        ...
        const nftMap = await Promise.all(
            collections.map(async (coll, i) => {
                ...
                console.log("GET ALL NFTS", i, options, "0xf30536ea9bcC6F98e6D66f52c7104522C234E2D0");
...
function createMyInfoStore(): MyInfoStore {
    ...
    const getMyNFTs = async () => {
        ...
        const nftMap = await Promise.all(
            collections.map(async (coll, i) => {
                ...
                try {
                    ...
                    console.log("-=polynftsonNFTs", i, nfts);
        ...
        const withUpgrades = await Promise.all(flat.map(async (nft, i) => {
            ...
            return {
                ...nft,
                upgrades: upgrades.map((str, i) => ({
                    src: str.replace("ipfs://", "https://ipfs.io/ipfs/"),
                    index: i
                }))
            }
        }));
