<script lang="ts">
    ...
    const upgradeSelected = (i, upgr) => {
        selected = i;
        selectedUpgrade = upgr;
    }
    ...
    const acceptUpgrade = async () => {
        ...
        if (isConfirmed) {
            ...
            try {
                const tx = await DYContract.upgrade(
                    myNft.id,
    ...
    <div class="tab-pane fade  show active" id="tab-1" role="tabpanel">
        {#if myNft.id}
            ...
            <div class="asset__actions asset__actions--scroll" id="asset__actions--scroll">
                {#each myNft.upgrades as upgrade, i}
                    <div style="cursor:pointer;" on:click={() => upgradeSelected(i, upgrade)}
                         id="details"
